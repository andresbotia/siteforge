...
- Restaurant Builder drafts now render through Restaurant Modern V2. The renderer keeps the existing `restaurant-modern` template key but adds a richer one-page composition for sourced cuisine, public reputation, verified hours/location, approved local image assets, and no-image visual fallback. Image assets are structured in `WebsiteSpec.assets.images` with URL/reference, alt text, source type, source URL, rights status, attribution, operator approval status, and role. Only approved, rights-approved, allowlisted local restaurant fixture assets render; unapproved, unsafe, or third-party reference images fail closed. This does not publish a preview, send email, call paid AI, mutate Antojitos production data, or start M10.
