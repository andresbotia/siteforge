...
1. **`designer_jobs` and the `external_site_artifacts.provider` migrations are committed but not applied to the hosted Supabase project.** `npx supabase migration list` shows the remote history contains two versions (`20260831113741`, `20260831125533`) with **no matching local migration file** -- this session did not create this drift and does not know its origin (it might be legitimate prior work applied outside a committed migration file, e.g. through the dashboard, or something else). `npx supabase db push --dry-run` fails closed with `LegacyDbPushMissingLocalError` and suggests either `supabase migration repair --status reverted 20260831113741 20260831125533` (rewrites remote migration history bookkeeping) or `supabase db pull` (would pull those two into local files). Per this repo's safety rules, this session did not run either without operator review, since both are consequential, hard-to-fully-reverse actions against the same production database that already holds real Resend configuration and is heading toward a real prospect campaign. **Operator action needed:** inspect what those two remote migrations actually are (Supabase dashboard, or `supabase db diff` against a fresh pull) before deciding whether to `repair` or `pull`, then push the two new migrations from this session.
2. **No Docker on this machine**, so there is no local Postgres to test the real persistence path against as an alternative. The smoke test proves everything up to and including "candidate is built and independently validated," but the final `finalizeDesignerJobSuccess()` write path (`src/lib/designer/worker-db.ts`) has type-checked and is exercised by the state-machine tests, but has **not** been run against a real database in this session.
...
6. Nothing in this session was pushed to `origin/main`. AGENTS.md rule 28 ("Do not autonomously push commits ... unless explicitly instructed") takes precedence over the session prompt's more general "push if that remains normal practice" -- ask before pushing.
