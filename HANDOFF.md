...
For the next session. Milestones 1 through 9 are locked, with the latest M9.5A readiness lock at `bfbf41181fb8c1c1ba3ba56ab38f5c2606b8f007`. M9.5B real-prospect preparation and Auditor calibration are locked, with the M9.5B Auditor Calibration lock at `1358caad47c46b9832f875ec1e62d5834043906b`. M9.5C guarded real email integration/internal send is complete: Resend is configured server-side, the sending domain was verified externally, the live-email gate was exercised for one operator-only test, and the test delivered without prospect/customer funnel mutation. M9.5D first controlled prospect campaign preparation is current. The operator deferred credential rotation for now; credential rotation is still required before sensitive customer/payment data, live payment use, or broader production operation. M10 (Operator Console -- navigation + information architecture) is complete. M10.5 (Visual System Pass) is complete, though it shipped dark and M10.6 converted it to light. M10.6 (Data hygiene and light theme) is current. The archive-stale-leads script's `--execute` was run against the real hosted database in a follow-up to this session (after fixing an env-loading bug and an npm arg-passthrough bug): **17 leads archived, 5 stale approvals expired.** All real-provenance leads (Antojitos, the four manual HVAC/plumbing/electrical prospects, "SiteForge Stripe Test") are confirmed still present and unarchived. See the addendum at the end of the M10.6 session entry below.
...
- **Done, no longer pending:** the Task 1 archive was reviewed and executed in the addendum below (`npm run leads:archive-stale:execute`) -- 17 leads archived, 5 approvals expired, independently re-verified.
- The "SiteForge Stripe Test" lead (Stripe TEST-mode customer, business name is self-declaratory) is still flagged but not archived -- it doesn't match the mock-identifier rule as written. Decide by hand whether to archive it via the console (`/leads/[id]` -> Lifecycle) or extend the script.
...
### Addendum -- archive script fixes and --execute

Two bugs surfaced when the operator actually ran `npm run leads:archive-stale`:

1. **No env loading.** The Next.js app gets `.env.local` loaded automatically; a standalone `tsx` process does not. Fixed with a `loadDotEnvLocal()` helper in the script using Node's own built-in `process.loadEnvFile()` (no new dependency) -- a no-op, not an error, when the file doesn't exist, and it never overrides a variable already set in the real environment (same "shell wins" convention as `node --env-file`). The generic "both must be set" error was also replaced with `getSupabaseServerConfigIssueFromEnv()`'s specific per-variable message.
2. **`npm run leads:archive-stale -- --execute` glued the flag onto the script path with no separating space** on the operator's machine (not reproduced in this session's own shells, so the exact mechanism is unconfirmed -- possibly a different npm version or shell than the one available here). Rather than chase a mechanism I couldn't reproduce, added a dedicated `npm run leads:archive-stale:execute` script (`package.json`) that never needs arg passthrough at all, and made the script's own argv check tolerant of a glued flag too (`arg.endsWith("--execute")` as well as an exact match) as a second line of defense. `-- --execute` still works if preferred; the dedicated script is the reliable path.

Verified both fixes, then ran the dry run again (identical classification to the one in this reply) and, per instruction, `npm run leads:archive-stale:execute`:

**17 leads archived**, each with its category-specific reason recorded in `archived_reason`: 14 seed/fixture, Tiny Leak Bros + Verdant Lyfe (scout, never advanced), Atlantic Drain Plumbing (mock Stripe customer). **5 approvals expired** (Palmetto/Oakridge `website_deployment`, Harborline `external_email`, Tidewash `payment_action` + `external_email`) -- confirmed by an independent read-only query after the run, which also found 2 unrelated approvals already `expired` from before this session (not touched by this script).

**Independently re-verified afterward** (read-only queries, not just the script's own log): re-running the dry run shows `0 to archive` against the remaining 8 leads. Those 8, confirmed by name: **Signature Air Conditioning & Heating**, **Joe & Joe Air Conditioning**, **Powercom LLC**, **Pam Blount Plumbing & Supplies**, **Antojitos Salvadoreños Restaurant** (all `manual_public_prospect`, all untouched), **SiteForge Stripe Test** (still flagged, still not archived -- TEST-mode Stripe id, not mock), and the two Scout `rejected` leads (Perfect Choice Nursery, The Time Is Now Design & Build) left alone per the judgment call above.
